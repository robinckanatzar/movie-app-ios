...
class ClientHTTP: ClientHTTPProtocol {
    ...
    private enum Constant {
        ...
        static let preprodFirebaseURL = "https://example-chat-e86a5.firebaseio.com/"
    }
    ...
    // Firebase
    private func fullFirebaseURL(with path: String) -> URL {
        #if PREPROD
        let baseURL = Constant.preprodFirebaseURL
        #else
        let baseURL = Constant.firebaseURL
        #endif
    ...
    // Firebase
    private func makeCallFirebase(on path: String, httpMethod: HTTPMethod, parameters: Parameters?,
                                  encoding: ParameterEncoding?, completionHandler: @escaping (DataResponse<Any>) -> Void) {
        let url = fullFirebaseURL(with: path)
        print("url = \(url)")
        var finalEncoding: ParameterEncoding = URLEncoding.default
        if let encoding = encoding {
            finalEncoding = encoding
        ...
        Alamofire.request(url, method: httpMethod, parameters: parameters, encoding: finalEncoding).responseJSON { (response) in
            completionHandler(response)
        }
    }
    ...
    func makeRequestFirebase(on path: String, httpMethod: HTTPMethod, parameters: Parameters?,
        ...
        print("path = \(path)")
        makeCallFirebase(on: path, httpMethod: httpMethod, parameters: parameters, encoding: encoding) { (response) in
            let statusCode = response.response?.statusCode ?? 200
            let statusCodeIsSuccessful = [200,201].contains(statusCode)
            completion(ServiceResult.success(value: response.result.isSuccess && statusCodeIsSuccessful))
        }
